...
原代码在查找"最后一个任务"时，会跳过包含 `[completed]` 的已完成任务：
...
当文档中 R14 是 `[completed]`，而新任务 R15 不存在时，代码会：
1. 从文件末尾向前遍历
2. 遇到 R14（带 `[completed]`），因为条件 `!line.includes('[completed]')` 为 false 而跳过
3. 继续向前找到 R13
4. 在 R13 之后插入新任务 R15
...
移除所有 `!line.includes('[completed]')` 条件，使用更精确的任务标题匹配规则（检查是否包含 RXX 格式）。
