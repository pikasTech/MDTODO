...
R26 和 R26.1 实现了任务完成状态的标记功能，但后续代码被回滚（"代码意外回滚了" commit），导致 `handleMarkComplete` 方法变成了一个仅显示通知的存根（stub），没有实际修改 markdown 文件中的 `[completed]` 标记。
...
/**
 * 设置任务的 [completed] 状态
 * 添加或移除 [completed] 标记，支持处理 [Processing] 标记
...
private async markTaskAsFinished(taskId: string, isFinished: boolean): Promise<void> {
  ...
  // 添加或移除 [completed] 标记
  if (isFinished) {
    // 在任务ID后添加 [completed]，并移除 [Processing] 标记
    let newLine = line;
...
// 添加 [completed]，在可能存在的 [Processing] 之前
const taskIdPattern = new RegExp(`(${taskId.replace(/\./g, '\\.')})(\\s*\[)`);
...
1. **普通任务完成标记**：点击复选框添加/移除 `[completed]` 标记
2. **Processing 状态下标记完成**：
   - 任务处于 `[Processing]` 状态
   - 点击复选框
   - 预期：`[Processing]` 标记被移除，`[completed]` 标记被添加
3. **已完成任务取消完成**：点击已标记完成的复选框移除 `[completed]` 标记
4. **边界情况**：状态相同时不做修改
