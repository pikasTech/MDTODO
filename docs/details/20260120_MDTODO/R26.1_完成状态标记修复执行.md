...
R26 修复完成后，完成状态标记功能又不能使用了。点击复选框后只显示通知消息，但没有实际修改 markdown 文件中的 `[completed]` 标记。
...
2. **新增 `markTaskAsFinished`**：实现添加/移除 `[completed]` 标记的逻辑
...
private async markTaskAsFinished(taskId: string, isFinished: boolean): Promise<void> {
  ...
  // 添加或移除 [completed] 标记
  if (isFinished) {
    // 在任务ID后添加 [completed]，在可能存在的 [Processing] 之前
...
1. **状态切换**：点击未完成任务的复选框会添加 `[completed]` 标记，点击已完成任务的复选框会移除该标记
...
1. 打开一个 TODO 文件
2. 点击任务的复选框
3. 检查文件中的任务标题是否正确添加/移除了 `[completed]` 标记
