...
在 `webviewProvider.ts` 中，`handleMarkComplete` 方法（第570-572行）只是显示了一个通知消息，但没有实际修改 markdown 文件中的 `[completed]` 标记：
...
- **添加 [completed]**：在任务ID后添加标记，支持处理 `[Processing]` 标记的替换
- **移除 [completed]**：从任务行中移除标记
