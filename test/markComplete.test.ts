...
describe('完成任务标记功能测试', () => {
  ...
  function toggleFinishedMark(content: string, taskId: string): string {
    ...
    if (hasFinished) {
      ...
      if (hasProcessing) {
        ...
        lines[taskLineIndex] = line.replace(/\s*\[Processing\]/, ' [Finished]');
      } else {
  ...
  describe('添加 [Finished] 标记', () => {
    test('应为未完成的任务添加 [Finished] 标记', () => {
...
## R2 任务二`;
      ...
      expect(result).toContain('## R1 [Finished] 任务一');
      expect(result).toContain('## R2 任务二');
    });

    test('应为子任务添加 [Finished] 标记', () => {
      const content = `## R1 任务一
...
## R2 任务二`;
      ...
      expect(result).toContain('### R1.1 [Finished] 子任务');
    });
...
## R2 任务二`;
      ...
      expect(result).toContain('## R2 [Finished] 任务二');
    });
...
## R2 (已完成)`;
  ...
  describe('移除 [Finished] 标记', () => {
    test('应为已完成的任务移除 [Finished] 标记', () => {
...
## R2 [Finished] 任务二`;
  ...
  describe('状态切换', () => {
    ...
    test('点击未完成任务的复选框应添加标记', () => {
      const content = `## R1 任务一`;
      const result = toggleFinishedMark(content, 'R1');
      expect(result).toBe('## R1 [Finished] 任务一');
    });
  ...
  describe('与 [Processing] 标记的交互', () => {
    test('有 [Processing] 时添加 [Finished] 应替换', () => {
    ...
    test('有 [Finished] 时添加 [Processing] 不应影响 [Finished]', () => {
      const content = `## R1 [Finished] 任务一`;
      const result = toggleFinishedMark(content, 'R1');
      // 移除 [Finished] 后不应添加 [Processing]
      expect(result).toContain('## R1 任务一');
...
## R2 任务二`;
      ...
      const r1Index = lines.findIndex(l => l.includes('R1 [Finished]'));
